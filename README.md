...
**顺序别记反：逻辑审读在完稿之后。** B2 的常规轮次读的是正文、人物卡、节拍，核设定与因果——它是成稿后的角色。§4.0 在方案阶段还有一次**计划检查**（手里只有节拍表与人物卡，回答"主线每一跳有没有支点"），那是**为了把返工拦在动笔前**，既不是逻辑审读，也不能替代成稿后的 B2；两次用的是同一位 B2、同一个名额。
