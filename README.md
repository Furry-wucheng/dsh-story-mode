...
### 关系轴：B2 那一轮的另一半（v1.2.0，起点组 v1.2.1，v1.3.3 起与逻辑同轮）
...
所以关系轴**不另派**：它就是 B2 完稿后那一轮的另一个必答（v1.3.3 起；v1.2.0–v1.3.2 曾在方案阶段另派一次，已取消——那时没有正文，判不出可信结论）：

| 时机 | 读什么 | 必答 |
|---|---|---|
| **首次成稿后**（随 B2 的常规轮次，派发消息加一行"审读类型：关系轴"） | 正文 + 节拍、人物卡 | 主线每一跳的推动者；另一方的动心露出有几条、哪些**不可否认**；被动方有没有主动动作；**双方的注意起点落在哪一场哪一句、为什么只对这个人成立**；承诺的每条逐条给出正文短引，引不出来算缺 |
| **之后每一版** | 新正文 | 改过推动者、关系状态或补过拍时必做：上一版报的缺拍与旧条目逐条复核 |

**方案阶段不派独立读者**：节拍表的每一拍要写得出推动者、双方状态与代价，人物卡要写出双方的注意起点（第零组的两条判据）——这是主代理拟方案时的硬性自查，不是审读；交付方案时要注明"未经独立审读"。
...
| B2 故事逻辑 | `subagent_review_b2` | 正文、作者有效要求、人物卡、节拍、必要前文与设定 | 每版都派。**逻辑审读与关系轴（§4.0）是它的同一轮**，都在完稿之后；方案阶段不派 |
...
回归测试覆盖分场、引号统计、真实配额、用词线索、人物卡和卸载检查，并钉住两类东西。**审读流程的形态**：角色各自成行、只读 toolFilter、B3/B4 的可观测触发条件、人设里不再夹带流程规则，以及 **v1.2.0 的关系轴契约**（B2 的两组必答项、"全可否认＝零"判据、节拍表的推动者三列、"免确认"不能跳过关系轴）与 **v1.2.1 的起点组**（B2 的第零组、双方各一条注意起点、"只有叙述者说得出的理由＝没有理由"、缺的起点补在开篇之前，以及"什么时候可以留白、为什么是这个人不能"）；**v1.3.3 的时机**也钉住了：逻辑审读与关系轴是同一轮、都在完稿之后，方案阶段不派独立读者。**0.1.7 的组合形态**：patch 是 `insert` 层、插件行用裸包名、包内路径用 `createRequire(baseUrl)`、两份技能只从包内挂载、组合里该有的行一行不少。测试只使用内存稿件和临时 DSH 主目录，不修改真实用户设置。
