...
先区分完整故事与片段、单场景、开头、对话，再按范围新写、续写、修改或评价。局部创作不强制新建接稿单或启动完整审读面板。
...
先明确创作方向、内容重点、人物关系、基调与范围，再选择新写、续写、修改或评价的对应流程。缺失信息影响当前交付、且无法从上下文或授权合理决定时，一次问完必要问题；存在明显不同的方向时让作者确定后再写，片段也适用。已给出的要求直接沿用，授权自拟的内容直接决定；方向明确且已授权写作时直接执行。作者要求先看方案时再等确认。

- **新写片段、单场景、开头或对话**：方向明确后按范围创作，完成指定内容即停止；不追问全篇结局、套用短篇最低字数或擅自补成完整故事。
- **新写完整故事**：按需读取完整故事流程，按授权规划和成稿，先冷读，再修订，按需专项审读，最后由一位全新读者盲读新版。
- **续写**：在获准材料内沿用前文和有效设定；只续一段就走局部流程，续写完整篇章才用完整故事流程。遵守指定停止位置。
- **整体改稿**：先读现稿，按问题选择结构、阅读体验或文风审读，不为套模板重建全部文件。
- **局部润色**：读指定片段及必要上下文，修改后检查接缝和信息，简短交付。不会自动扩大为整篇改写。
- **只要评价**：只给意见，不改正文。

完整作品的篇幅档位仅供参考：微型 3k–5k、标准短篇 5k–10k、中短篇 10k–20k、系列每篇 3k–10k。作者给出具体字数时直接采用；片段没有这些最低字数要求，字数本身也不决定任务是否属于完整故事。

### 故事资产

沿用现有目录和文件名。完整故事确有规划需要时，用 brief.md 保存有效约束、outline.md 保存场景计划、draft.md 保存正文。局部创作通常只需正文文件；新开目录不意味着建立整套故事资产，也不授权读取其他故事目录。
...
完整故事默认规模只有两位读者，不要求每次都派四路。局部任务默认自查；作者要求独立审读时，只检查对应片段，不因未写完整故事而要求补结局。
...
写作入口按需读取 references/full-story-workflow.md（完整故事）和 references/review-panel.md（独立审读）；两份资料都位于 presets/short-story/skills/short-story/ 下，不是新的独立技能。
...
dsh-story-mode/
  ...
  presets/short-story/              模式本身：agent.cordis.yml + 元数据 + 写作流程技能
    skills/short-story/             SKILL.md：任务分流、局部创作与共同规则
      references/                  full-story-workflow.md：完整故事流程；review-panel.md：审读模板
