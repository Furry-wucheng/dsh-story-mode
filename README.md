...
新写、续写或大幅重写，包括片段，一般是：B1 初次冷读 + B2 人物卡与节拍核对 → 主代理修改 → 原读者复核 → 全新 B1 最终盲读。B3/B4 按专项问题增加，B1/B2 不预先看彼此报告。
一句或一段的润色也至少有一位独立审读员检查改动与接缝。自查、计数和 lint 不等于独立审读；作者催快时缩短报告，不默认取消审读。不可用时说明实际失败并标待审，不虚报验收。
