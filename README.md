...
dsh plugin --profile <你的 profile> add ./dsh-story-mode-1.1.0.tgz
```
...
直接说明要新写、续写、改整篇还是改某一处。已给过的字数、风格和结构要求直接沿用，不重复问。

- **新写完整故事**：明确必要约束，给简洁方案；按授权成稿，先冷读，再修订，按需专项审读，最后由全新读者盲读新版。
...
完整新稿或整体修订一般是：初次冷读 → 主代理修改 → 按需专项审读 → 修改 → 最终盲读。
局部任务只做对应范围的检查；不要求每次都派四路。
