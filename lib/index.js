 ...
 *   preset/agent.cordis.yml                   **模式本身**（人设 + 工具行 + 流程入口）
 *   preset/preset.yml                         模式在 roster 里的显示名与描述
 *   preset/skills/short-story/SKILL.md        写作流程技能（形态识别 / 档位 / 四阶段）
 *   skills/writing-style-contract/SKILL.md    文风契约（全局技能，非写作模式也生效）
 *   scripts/install-links.mjs                 把包接到 DSH 的发现根上（或只做检查）
 *   cordis.patch.yml                          可选的 profile bundle 通道
 *
 ...
 *
 * "整套"是靠 `scripts/install-links.mjs` 把包内内容**复制**到 DSH 的两个固定
 * 发现根（`<DSH_HOME>/.agent-presets/<id>/` 与 `<DSH_HOME>/skills/<name>/`）
 * 实现的。为什么必须复制、不能符号链接，见那个脚本的文件头——那是一处
 * 框架内部判定不一致导致的坑（模式侧 `readdir().isDirectory()` 不跟随链接）。
 *
 * 为什么必须是复制落地，而不是更省事的自动机制：
 *   * profile bundle（`dsh.bundle.patch`）只往**宿主组成**插行，而 preset 属于
 *     agent 平面、由 roster 从文件系统根发现——bundle 无法创建模式；
 *   * preset 行的 `name` 不能用 `!!js` 动态算路径：发现阶段确实用 loader 的
 *     YAML 方言解析（支持 `!!js`），但随后的形状检查要求 `name` 是字符串，
 *     `!!js` 解析出来是对象，整份组成会被判为 broken；
 *   * 裸包名从 harness 解析，而 preset 在用户目录下，向上查找到不了 harness
 *     自己的依赖。
 * 三条路都堵死，落地就是唯一机制——所以 `story_doctor` 是它的必要配套。
 *
