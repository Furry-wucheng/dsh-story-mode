 ...
 *
 * 这是为了让"叙述里出现了什么"可以被确定性地问出来：台词里的"你/我"是人物在
 * 说话，不是视角滑移。涂掉而不是删掉是为了不动行号与字符位置。
 *
  ...
  return text
    ...
    .map((line) => {
      ...
      for (const char of line) {
        ...
        out += MASK_CHAR
        if (char === QUOTE_CLOSER.get(open)) open = null
      }
      return out
    })
    .join('\n')
...
function repeatedNarrationWords(paragraphs, threshold) {
  ...
  for (const paragraph of paragraphs) {
    if (isDialogueParagraph(paragraph.text)) continue
...
function analyze(body, tierId, config) {
  ...
  const dialogueParagraphs = paragraphs.filter((paragraph) => isDialogueParagraph(paragraph.text))
  const dialogueChars = dialogueParagraphs.reduce((sum, paragraph) => sum + countText(paragraph.text), 0)
  // 一次算完并舍入，报告与提示读同一个数。
...
function renderLint(report) {
  ...
  for (const finding of report.findings) {
    ...
    for (const hit of finding.hits.slice(0, 6)) {
      // 视角那条会在一份报告里混装三类信号，所以每条命中自带标签。
      const label = hit.label === undefined ? '' : `［${hit.label}］`
      lines.push(`- 第 ${hit.line} 行：${label}${hit.text}`)
    }
...
const RULES = [
  ...
  },
  {
    ...
    title: '对话提示语过密（契约五）',
    rule: '`他说` / `她说道` / `他开口` 这类提示语。说话人明确时直接写台词；需要区分时优先用自然动作。只统计同一行里既有引号、又落在引号外的提示语。',
  ...
  },
  {
    ...
    title: '情绪修饰式提示语（契约五）',
    rule: '`他说，声音平静得…` / `她轻声说道` 这类把情绪塞进提示语的写法。改成动作，或者直接删掉提示语。只统计同一行里既有引号、又落在引号外的写法。',
...
/** 报告顺序：视角那条夹在契约五与契约十之间，与契约条款顺序一致。 */
const RULE_ORDER = [
  'template-simile',
...
const POV_RULE = [
  '只看**引号外**的叙述（台词里的"你/我"是人物在说话，不算），判两件事：',
  '① 第一人称与第二人称同时出现在叙述里——这是真正的滑移，任何视角下都报；',
  '② 全知视角词（`与此同时` / `殊不知` / `另一边`）出现在叙述里。',
  '传 `pov: third-limited` 时更严：叙述里出现"我"或"你"任一个都报（限知第三人称本不该有这两个人称）。',
  '传 `pov: first` 时只报 ②，"你"按称呼容忍；传 `pov: omniscient` 则不报 ②。',
].join('')
...
/**
 * 视角滑移：不按正则逐行扫，而是先算出"叙述里的三类信号"，再按视角策略决定报不报。
 *
 * 这条规则以前是 `/你(?=[\u4e00-\u9fff])/`——只要句子里有"你"就算滑移，于是第三
 * 限知叙事里每一句对白都会命中。假警报比漏报贵得多：报告一旦不可信，作者就会
 * 开始整条跳过它，那这个工具就白装了。
 */
function collectPovFinding(text, pov, maskedLines, rawLines) {
  const first = []
  const second = []
  const omniscient = []
  for (let i = 0; i < maskedLines.length; i += 1) {
    const line = maskedLines[i]
    // 整行都在引号里（纯台词）、空行、标题行：没有可判的叙述。
    if (!CJK_CHAR.test(line)) continue
    const hit = { line: i + 1, text: excerpt(rawLines[i]) }
    if (/我/.test(line)) first.push({ ...hit, label: '叙述里出现第一人称' })
    if (/你/.test(line)) second.push({ ...hit, label: '叙述里出现第二人称' })
    if (/与此同时|殊不知|另一边[，,]/.test(line)) omniscient.push({ ...hit, label: '全知视角词' })
  }
  ...
  // 全知叙事里"另一边""与此同时"是正常调度，不是滑移。
  if (pov !== 'omniscient') hits.push(...omniscient)

  if (hits.length === 0) return null
  hits.sort((a, b) => a.line - b.line)
  return {
    id: 'pov-drift',
    title: '视角滑移（接稿单的「视角」字段）',
    rule: POV_RULE,
    hits,
    total: hits.length,
  }
 ...
 * 逐条跑规则表。
 *
 * 同一行内的多处命中合并为一条，取整行文本作为样本——这样报告读起来像改稿
 * 意见，而不是正则结果。
 */
function collectFindings(text, signal, pov) {
  const lines = text.split(/\r?\n/)
  const maskedLines = maskQuotedSpans(text).split('\n')
  const findings = []

  for (const rule of RULES) {
    const scoped = rule.scope === 'narration'
    const source = scoped ? maskedLines : lines
    const hits = []
    let total = 0
    for (let i = 0; i < source.length; i += 1) {
      signal?.throwIfAborted()
      // 提示语只有挨着台词才叫提示语：这一行必须真的有引号。
      if (scoped && !ANY_QUOTE.test(lines[i] ?? '')) continue
      const matches = source[i].match(rule.pattern)
      if (matches === null || matches.length === 0) continue
      total += matches.length
      hits.push({ line: i + 1, text: excerpt(lines[i]) })
    }
    if (hits.length > 0) findings.push({ id: rule.id, title: rule.title, rule: rule.rule, hits, total })
  }

  const povFinding = collectPovFinding(text, pov, maskedLines, lines)
  if (povFinding !== null) findings.push(povFinding)

  return findings.sort((a, b) => RULE_ORDER.indexOf(a.id) - RULE_ORDER.indexOf(b.id))
}
...
function makeWordcountTool(ctx, config) {
  return makeTool({
    ...
    description: [
      ...
      'dialogue-vs-narration ratio, paragraph lengths, and two-character words repeated within a single narration paragraph.',
...
function makeLintTool(ctx) {
  return makeTool({
    ...
    description: [
      ...
      'Dialogue tags only count on a line that actually carries quoted speech, and the POV rule only reads narration outside quotes —',
      'speech is a character talking, not the narrator\'s habit. POV is reported when first and second person mix inside narration, or when',
      'omniscient connectives appear there; pass `pov` (from brief.md) to make that judgement strict or relaxed.',
    ...
    parameters: {
      ...
      pov: { type: 'string', enum: POV_IDS, description: 'Point of view declared in the brief: first | third-limited | omniscient. Pass it when brief.md states one — third-limited makes the POV rule strict (any 我/你 in narration is reported), omniscient stops reporting omniscient connectives. Omit it and only the hard signal is reported: first- and second-person pronouns mixed inside narration.' },
    },
    presentCall: (args) => ({ card: 'generic', title: `文风检查 ${args.path}`, kind: 'read', rawInput: args.path }),
    async run(args, exec) {
      const target = await resolveTarget(ctx, args.path, exec)
      const raw = await readTarget(ctx, target, exec)
      let findings = collectFindings(raw, exec?.signal, args.pov)
