...
function buildNotes({ total, tier, sceneStats, targets }, config) {
  ...
  if (tier && total < tier.min) notes.push('总字数 ' + total + '，比「' + tier.label + '」参考下限 ' + tier.min + ' 少 ' + (tier.min - total) + ' 字；档位仅供参考。')
  if (tier && total > tier.max) notes.push('总字数 ' + total + '，比「' + tier.label + '」参考上限 ' + tier.max + ' 多 ' + (total - tier.max) + ' 字；档位仅供参考。')
  if (targets === null) {
 ...
 * 两个入口共用一个实现，因此不会漂移。这里把 `ctx` 传下去，自检因此在报告里
 * 附上活体检查（roster / 本作用域的技能与工具）——它跑起来的这个事实本身，
 * 就已经证明本插件的四个工具在这个作用域里注册成功了。
 */
