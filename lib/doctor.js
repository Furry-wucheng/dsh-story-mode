...
export async function runDoctor() {
  ...
  // 写作入口必须读取共用写作流程与审读面板，缺失参考文件会中断流程。
  const presetWritingWorkflowPresent = await exists(
    join(root, 'presets', PRESET_ID, 'skills', PRESET_ID, 'references', 'writing-workflow.md'),
  )
  ...
  // 五个审读角色必须各自成行，并且**人设与只读限制都在组合里**。退回 v1.1.2 那种
  // "一个通用 subagent + 面板里手抄模板"的形态不会报错，只会让信息边界变成每次派发
  // 都由主代理复述——复述一次就可能漏一条，而且只读退回成提示词约束。
  ...
  const reviewerRows = REVIEWER_IDS.map((role) => ({
    role,
    toolName: presetComposition.includes('toolName: subagent_review_' + role),
    persona: presetComposition.includes("reviewers/" + REVIEWER_FILE[role]),
    readOnly: presetComposition.includes('allow: [read, read_image, str_replace_editor, glob, grep]'),
    deniesWrites: presetComposition.includes('deny: [write, edit, present,'),
  }))
  const presetReviewersRoleBased = reviewerRows.every((row) => row.toolName && row.persona && row.readOnly && row.deniesWrites)
  const presetReviewerFilesPresent = (await Promise.all(
    REVIEWER_IDS.map((role) => exists(join(root, 'presets', PRESET_ID, 'skills', PRESET_ID, 'references', 'reviewers', REVIEWER_FILE[role]))),
  )).every(Boolean)
...
export function renderDoctor(report) {
  ...
  if (!report.viaProfile) {
    ...
    lines.push(`| 共用写作流程（写作技能 references/） | ${report.presetWritingWorkflowPresent ? '是' : '**否 —— 写作流程无法加载**'} |`)
    lines.push(`| 审读员可复用（continuable + send_message） | ${report.presetReviewersReusable ? '是' : '**否 —— 每位审读员都会退化成一次性会话，改一轮就要重读全文**'} |`)
    lines.push(`| 五个审读角色各自成行（persona + 只读 toolFilter） | ${report.presetReviewersRoleBased ? '是' : '**否 —— 角色人设或只读限制没进组合，退回成每次派发口头复述**'} |`)
    lines.push(`| 角色人设文件（references/reviewers/） | ${report.presetReviewerFilesPresent ? '是' : '**否 —— 组合里的 !!js 会读不到文件，整个 preset 挂不上**'} |`)
    ...
    if (!report.presetWritingWorkflowPresent) problems.push('包内缺少写作技能的 references/writing-workflow.md（共用写作流程）')
    ...
    if (!report.presetReviewersRoleBased) problems.push('审读角色没有各自成行（缺 persona 或只读 toolFilter），派发边界会退回口头复述')
    if (!report.presetReviewerFilesPresent) problems.push('包内缺少审读角色人设文件（references/reviewers/*.md）')
