...
export async function runDoctor() {
  ...
  // 审读面板是写作流程技能的第二份文件（同目录 `references/`）。技能正文按相对路径引用它，
  // 缺了它流程会在"派审读员"那一步悬空——不是故障，但等于少掉一半的改稿依据。
  ...
  const presetStyleSkillMounted = (
    await readIfPresent(join(root, 'presets', PRESET_ID, 'agent.cordis.yml')) ?? ''
  ).includes("new URL('../../skills/', baseUrl)")
...
export function doctorTool() {
  return makeTool({
    ...
    description: [
      ...
      'predicate (readdir + isDirectory — a symlink reads fine but the roster skips it), that the contract is mounted, and',
